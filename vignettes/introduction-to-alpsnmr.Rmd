...
building pipelines with this package, you may want to open the file saved in
this directory (run it on your computer):
...
As we already have the `NMRExperiment` column, we can use it as the merging column:
...
Now we have the `SubjectID` information so we can add the table that adds
the `SubjectID` to the `Age`.
...
be embedded in RMarkdown documents. Additionally, some old web browsers may
not be able to display these interactive plots correctly.

```
plt <- plot(dataset, NMRExperiment = c("10", "30"), chemshift_range = c(2.2, 2.9))
...
The AlpsNMR package includes some robust PCA analysis for outlier detection.
...
Our current approach relies on the use of the Signal to Noise Threshold (`SNR.Th`)
to discriminate valid peaks from noise, however a global intensity threshold can
also be used. The advantage of the `SNR.Th` method is that it estimates the noise
...
peak_table <- nmr_detect_peaks(dataset,
                               nDivRange_ppm = 0.1,
                               scales = seq(1, 16, 2),
                               baselineThresh = 0, SNR.Th = 3)
nmr_detect_peaks_plot(dataset, peak_table, NMRExperiment = "10", chemshift_range = c(3.5,3.8))
```
...
```{r}
plot(dataset, chemshift_range = c(2.3, 2.5))
plot(dataset_align, chemshift_range = c(2.3, 2.5))
```


# Normalization

There are multiple normalization techniques available. The one providing the
best result on our samples has been the `pqn` normalization, but it may not be
fully reliable when the number of samples is very small, as it needs a reliable
...
The `AlpsNMR` package offers the possibility to extract additional diagnostic
information with `nmr_diagnose(dataset)`. This feature is only used for now
on the normalization, to explore the normalization factors applied to each sample:
...
## Integration based on peak boundaries

Imagine we want to integrate the four peaks corresponding to the pyroglutamic
acid:

```{r}
pyroglutamic_acid_region <- c(4.15, 4.20)
plot(dataset_norm, chemshift_range = pyroglutamic_acid_region) +
  ggtitle("Pyroglutamic acid region")
```
...
## Integration based on peak center and width

If instead of ranges we have a peak position, we can also integrate with the
peak position and some arbitrary width:

```{r}
nmr_data(
  nmr_integrate_peak_positions(dataset_norm,
