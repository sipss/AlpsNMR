...
test_that("get_peak_bounds returns NA when there is no left inflection point", {
...
test_that("get_peak_bounds returns NA when there is no right inflection point", {
...
test_that("get_peak_bounds returns non-NA bounds in the normal case", {
    result <- get_peak_bounds(
        peak_limit_left = 1,
        peak_limit_right = 3,
        pos = 2,
...
test_that("get_norm_rmse output changes when the apex arguments are swapped", {
    # Confirms the function is sensitive to which value is which, i.e.
    # y_fitted_apex and y_apex are not interchangeable.
...
test_that("peaklist_fit_lorentzians calls get_norm_rmse with all 4 arguments named", {
    # Guards against a future call passing arguments positionally, which
    # could silently bind them to the wrong parameters. We inspect the
    # actual function body rather than the file text, so this survives
    # reformatting.
...
test_that("refine_lorentzian_fit_with_nls leaves error_msgs empty when nls succeeds", {
    # A clean, well-sampled lorentzian (plus a tiny amount of noise, since a
    # perfectly noiseless exact fit can itself confuse nls's relative-offset
    # convergence criterion) should let nls converge, so no error should be
    # recorded.
    set.seed(42)
...
test_that("peaklist_fit_lorentzians accumulates peak_id and error_msg in lockstep", {
    # peak_id and error_msg are both grown by c() on every loop iteration, so
    # they must stay the same length even when multiple peaks hit the error
    # path. To trigger the error path without a full nmr_dataset pipeline, we
    # build a minimal 1-sample nmr_dataset_1D with two well-separated, clean
    # lorentzian peaks (so get_peak_bounds() finds real, non-NA bounds via
    # the raw signal's inflection points) but supply a jagged, non-lorentzian
    # "baseline" (data_1r_baseline) inside each peak's fitting window. Since
