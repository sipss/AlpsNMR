...
test_that("analyze_signal_regions_1d reports a much larger amplitude for a region with taller peaks", {
    ...
    expect_true(!is.na(tall_region$amplitude) && !is.na(small_region$amplitude))
    expect_true(tall_region$amplitude > 10 * small_region$amplitude)
})
...
test_that("analyze_signal_regions_1d gives NA amplitude for a region with no detected peaks", {
    ...
    result <- analyze_signal_regions_1d(y, num_regions = 4)
    expect_true(all(is.na(result$regions$amplitude)))
...
test_that("pool_signal_stats_regions_1d pools amplitude (median) across signals", {
    ...
    expect_true("amplitude" %in% colnames(pooled$regions))
    expect_true(!is.na(pooled$regions$amplitude[1]))
    expect_true(pooled$regions$amplitude[1] > 50) # roughly tracks 0.35^-1 * median(100,200,300)-ish scale
})
...
test_that("merge_sparse_regions_1d weights amplitude by each member's own peak count", {
    regions <- data.frame(
        ...
        density = c(0.001, 0.02), fwhm_q1 = c(5, 5), fwhm_q3 = c(10, 10),
        amplitude = c(1000, 100)
    )
    ...
    expect_true(merged$amplitude < 200)
    expect_true(merged$amplitude > 100)
})

test_that("merge_sparse_regions_1d defaults amplitude to NA when the input regions table lacks it", {
    regions <- data.frame(
        region = 1:2, frac_lo = c(0, 0.5), frac_hi = c(0.5, 1),
        density = c(0.1, 0.1), fwhm_q1 = c(5, 5), fwhm_q3 = c(10, 10)
    )
    merged <- merge_sparse_regions_1d(regions, n_total = 1000, min_peaks = 1)
    expect_true("amplitude" %in% colnames(merged))
    expect_true(all(is.na(merged$amplitude)))
})

## gen_synthetic_1d_regions: region-specific amplitude ---------------------------

test_that("gen_synthetic_1d_regions uses region_profile$amplitude when present", {
    region_profile <- data.frame(
        region = 1:2, frac_lo = c(0, 0.5), frac_hi = c(0.5, 1),
        density = c(0.05, 0.05), fwhm_q1 = c(5, 5), fwhm_q3 = c(10, 10),
        amplitude = c(10000, 10)
    )
...
test_that("gen_synthetic_1d_regions falls back to A when amplitude is absent (unchanged behaviour)", {
    ...
    set.seed(5)
    region_profile_na <- cbind(region_profile, amplitude = NA_real_)
