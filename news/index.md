...
- [`peaklist_fit_lorentzians()`](https://sipss.github.io/AlpsNMR/reference/peaklist_fit_lorentzians.md)
  (used by `nmr_detect_peaks(fit_lorentzians = TRUE)`): its “Fitting
...
- [`models_stability_plot_plsda()`](https://sipss.github.io/AlpsNMR/reference/models_stability_plot_plsda.md)
  ...
  the loadings-correlation heatmap now uses a fixed, sign-agnostic color
  scale (-1 and +1 both render as the same dark blue, 0 as white; a
  latent variable’s sign is arbitrary, so -1 and +1 indicate equally
  strong agreement) instead of a plain sequential palette that
  auto-scaled to whatever range the data happened to have. Also fixes
  self-correlation cells (expected to be exactly 1) intermittently
  rendering as blank/white: a loading vector’s self dot-product can come
  out as e.g. `1.0000000000000002` due to floating-point rounding, which
