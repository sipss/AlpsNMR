...
nmr_baseline_threshold <- function(nmr_dataset){
  ...
  threshold_ind = which((nmr_dataset$axis > range_noise_ppm[1]) | (nmr_dataset$axis < range_noise_ppm[2]))
  cent <- mean(apply(nmr_dataset$data_1r[, threshold_ind], 2, mean))
  disp <- 3*mean(apply(nmr_dataset$data_1r[, threshold_ind], 2, stats::sd))
