...
#' #' }
#' 
#' permutation_test_model = function (MVObj,
#'                                    nPerm = 50,
#'                                    nRep,
#'                                    nOuter,
#'                                    varRatio,
...
#'         warning(
#'             "Length of sampLabels not equal to number of samples in Y. \n     Autonumbers used instead."
#'         )
...
#'                          legend = c(
#'                              paste("Model R2 =", signif(MVObj$fitMetric$R2[modNum],
#'                                                         3)),
#'                              paste("Model Q2 =", signif(MVObj$fitMetric$Q2[modNum],
#'                                                         3))
#'                          ),
#'                          bty = "n")
#'     }
...
#'         if (is.null(ylim))
#'             ylim <- range(YPR)
#'         classes = seq_len(length(levels(Y)))
#'         if (missing(factCols))
#'             factCols = classes + 1
#'         if (length(factCols) != length(classes)) {
#'             warning(
#'                 "Length of factCols not equal to number of levels in Y. \n     Autocolors used instead."
#'             )
#'             factCols = classes + 1
#'         }
...
#'                        las = 3)
#'         for (cl in classes) {
#'             graphics::matpoints((seq_len(nSamp)) + classNudge[cl],
#'                                 YPR[, cl, ],
#'                                 pch = 20,
#'                                 col = factCols[cl],
#'                                 cex = 0.5
#'             )
#'             graphics::points((seq_len(nSamp)) + classNudge[cl],
#'                              YP[, cl],
#'                              pch = 20,
#'                              col = factCols[cl])
#'         }
#'         for (li in seq_len(nSamp + 1)) {
#'             graphics::abline(v = li - 0.5,
#'                              lty = 3,
#'                              col = "grey")
#'         }
...
#'         for (w in seq_len(length(wrongClass))) {
...
#'                     1),
#'             cex = 0.8,
#'             pt.cex = c(rep(0.5, length(classes)),
#'                        2),
#'             bty = "n"
#'         )
#'     }
...
#'             xlab = "Predicted Y"
#'         )
#'         graphics::points(YP, seq_len(nSamp), pch = 20, col = "black")
...
#'     X = a[seq_len(nrow(a)),] - b[seq_len(nrow(a)),]
