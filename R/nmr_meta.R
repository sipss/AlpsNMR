...
nmr_meta_add <- function(nmr_data, metadata, by = "NMRExperiment") {
  ...
  metadata <- do.call(dplyr::distinct, c(list(.data = metadata),
                                         rlang::syms(by),
                                         list(.keep_all = TRUE)))
...
nmr_meta_add_tidy_excel <- function(nmr_data, excel_file) {
  ...
  for (excel_df in excel_dfs) {
    ...
    avail_columns <- colnames(nmr_meta_get(nmr_data, groups = "external"))
    ...
    # Add the metadata
    nmr_data <- nmr_meta_add(nmr_data, metadata = excel_df, by = key_column)
  }
  nmr_data
}
