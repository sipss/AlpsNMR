...
tibble_lists_columns_to_vector_columns <- function(data) {
  ...
  # Missing values are replaced by their missing value type to workaround:
  # https://github.com/hadley/dplyr/issues/2203
  workaround_dplyr_issue_2203 <- function(data, to_simplify_cols, types) {
    # Replace a logical NA by the NA belonging to that class, being NA_character_ NA_integer_
    # or whatever. This should be taken care by dplyr inside unnest, but
    # there is a bug.

    # First get the class of each column to simplify. Any class is fine so we
    # get the first one of the list:
    types2 <- lapply(types[names(to_simplify_cols)], function(x) x[[1]])

    # We work on a copy of the data:
    data_with_typed_na <- data

    # For each column to simplify:
    for (i in seq_len(length(types2))) {
      # This is the column name
      colname <- names(types2)[i]
      # Its class:
      colclass <- types2[[i]]
      # A missing logical missing value
      colclassna <- NA
      # There are an infinite number of possible classes and I don't know
      # how to reliable create a missing value for that class.
      # It should be something like:
      # if (colclass == "character) {
      #    colclassna <- NA_character_
      # } else if (colclass == "numeric") {
      #    colclassna <- NA_real_
      # } ...
      # This is crazy, so we will try to find a generic approach based on trial
      # and error:
      success <- -1
      while (!is.na(success) && success < 0) {
        success <- tryCatch({
          if (success == -1) {
            colclassna <- vector(mode = colclass, length = 1)
            colclassna[1] <- NA
            1
          } else if (success == -2) {
            colclassna <- vector(mode = colclass[1], length = 1)
            colclassna[1] <- NA
            1
          } else if (success == -3) {
            colclassna <- structure(NA, class = colclass)
            1
          } else {
            NA
          }
      ...
      # If we did not succeed we crash
      # In case you reach this error visit: https://github.com/hadley/dplyr/pull/2209
      # And say that you need support for converting NA of class (colclass)
      # If the issue is merged, then you can use:
      # devtools::install_github(hadley/dplyr)
      # and try again
      if (is.na(success)) {
        stop("Could not create NA of type ", colclass)
      }
      # We have a good missing value so we have a go to replace NA with NA_character_
      # or whatever NA we have created:
      data_with_typed_na[[colname]] <- lapply(data_with_typed_na[[colname]], function(x) {
        if (is.na(x)) {
          colclassna
        } else {
          x
    ...
    return(data_with_typed_na)
  }
  data <- workaround_dplyr_issue_2203(data, to_simplify_cols, types)
