...
DATA_RELATED_FIELDS <- c("FIRSTX", "LASTX", "MAXX", "MINX", "MAXY", "MINY",
                         "XFACTOR", "YFACTOR", "FIRSTY", "LASTY", "DELTAX",
                         "DELTAY", "NPOINTS")
...
process_block <- function(lines, metadata_only = FALSE) {
  ...
  data_field <- NULL # we don't know the name of the data_field yet
  ...
  # Get the title:
  title <- sub(pattern = "^\\s*##TITLE\\s*=\\s*(.*?)\\s*$", replacement = "\\1", lines[1])
  block$TITLE <- title
  ...
  while (i < length(lines)) {
    ...
    name_val <- stringr::str_match(string = line, pattern = "^\\s*##(.*?)\\s*=\\s*(.*?)\\s*$")
    ...
    if (field_name == "TITLE") {
      ...
      new_block <- process_block(lines[i:length(lines)], metadata_only = metadata_only)
      block[["blocks"]] <- c(block[["blocks"]], list(new_block$block))
    ...
    if (field_name %in% DATA_FIELDS) {
      ...
      #################### (XY..XY) format ####################
      type_of_table <- stringr::str_match(string = field_value,
                                          pattern = "^\\s*\\(XY\\.\\.XY\\)\\s*(.*)\\s*")
      if (!is.na(type_of_table[1,1])) {
        ...
        if (!metadata_only) {
          ...
          block[[field_name]] <- data.frame(x = data[1,], y = data[2,], stringsAsFactors = FALSE)
        }
        next
      }
      ...
      if (!is.na(type_of_table[1,1])) {
        ...
        if (!metadata_only) {
          ...
          cleaned_spaces <- stringr::str_trim(lines[i_data_start:i])
          ...
          y <- unlist(lapply(strsplit(cleaned_spaces, split = "[[:blank:]]+"),
                             function(x) as.numeric(x[2:length(x)])))
          block[[field_name]] <- data.frame(x = NA, y = y, stringsAsFactors = FALSE)
        }
        next
      }
      # Unknown data format:
  ...
  block[fields_to_convert] <- lapply(block[fields_to_convert], convert_field)
  # Now that we have converted the fields we can create the X axis
  # for the (X++(Y..Y)) data format
  if (data_type == "(X++(Y..Y))") {
    x <- seq(from = block[["FIRSTX"]], to = block[["LASTX"]], length.out = block[["NPOINTS"]])
    block[[data_field]][["x"]] <- x
  }

  # Apply X and Y Factors to the data:
  if (data_type != "none") {
    if ("x" %in% names(block[[data_field]]) && "XFACTOR" %in% names(block)) {
      block[[data_field]][["x"]] <- block[[data_field]][["x"]] * block[["XFACTOR"]]
    }
    if ("y" %in% names(block[[data_field]]) && "YFACTOR" %in% names(block)) {
      block[[data_field]][["y"]] <- block[[data_field]][["y"]] * block[["YFACTOR"]]
    }
    # Convert x axis from frequency to chemshift
    if (tolower(block[["XUNITS"]]) == "hz") {
      block[[data_field]][["x"]] <- block[[data_field]][["x"]] / block[[".OBSERVE FREQUENCY"]]
    }
  }
...
read_jdx <- function(file_names, metadata_only = FALSE) {
  ...
  output <- plyr::llply(file_names,
                        function(file_name) {
                          lines <- readLines(file_name)
                          lines <- strip_comments(lines)
                          sampl <- process_block(lines,
...
create_df_from_jdx_sample <- function(sampl,
  ...
  for (block_idx in seq_along(sampl[["blocks"]])) {
    ...
    colnames(block_df) <- paste0("b", block_idx, "_", colnames(block_df))
